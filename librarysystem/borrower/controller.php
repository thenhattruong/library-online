...
switch ($action) {
   ...
   // SELECT `IDNO`, `BorrowerId`, `Firstname`, `Lastname`, `MiddleName`, `Address`, `Sex`, `ContactNo`, `CourseYear`, `BorrowerPhoto`, `BorrowerType`, `Stats`, `IMGBLOB` FROM `tblborrower` WHERE 1
	function doInsert(){
		...
		if ( $_POST['BorrowerId'] == "" OR $_POST['Firstname'] == ""
			OR $_POST['Lastname'] == "" OR $_POST['Address'] == "" 
			OR $_POST['ContactNo'] == "") {
			$messageStats = false;
		...
		if ( $_POST['BorrowerId'] == "" OR $_POST['Firstname'] == ""
			...
			redirect('index.php?view=add');
			 ...
			 //    $location = "photos/". $picture ;
				...
				message("Tài khoản đã được cập nhật!", "success"); 
		       redirect("index.php");
	    	} 
 ...
 function doChangeImage(){ 
			...
			if ($location!='photos/') {
			   ...
			   message("Hình ảnh đã được thay đổi!", "success"); 
		       redirect("index.php");
	    	}  
} 
...
if(isset($_POST['save'])){
		...
		$borrower->update($_SESSION['BorrowerId']); 
		  ...
		  message("Mật khẩu đã được thay đổi!", "success");
		  redirect("index.php");
	}
}
