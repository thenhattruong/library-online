...
if (isset($_POST['save'])){
		...
		if ($maxrow>0) {
			...
			$transaction->DueDate = date('Y-m-d', strtotime(date('Y-m-d'). ' + 7 day'));
...
function doConfirmed(){
	...
	$id = $_GET['id'];
		...
		message("Xác nhận đặt sách thành công!", "info");
		redirect('index.php');
...
function doReturned(){
	...
	$id = $_GET['id'];
		...
		message("Sách trả lại thành công!", "info");
		redirect('index.php');
...
function doCancelled(){
	$id = $_GET['id'];
		...
		message("Hủy giao dịch thành công!", "info");
		redirect('index.php');
}
